...
def validate_payload(
    ...
    *,
    ...
    if residual_fn is not None:
        try:
            summary["residual"] = residual_fn(payload)
    ...
    if diagnostics_fn is not None:
        try:
            summary["diagnostics"] = diagnostics_fn(payload)
        except Exception as e:  # noqa: BLE001
            ok = False
...
def main() -> None:
    ...
    result = validate_payload(payload, residual_fn=residual_fn, diagnostics_fn=diagnostics_fn)
    ...
    # Print compact, human-readable status first.
