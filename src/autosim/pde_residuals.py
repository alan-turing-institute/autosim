...
def kolmogorov_flow_diagnostics(
    payload: dict[str, Any],
    *,
    simulator: Any | None = None,
    simulator_kwargs: dict[str, Any] | None = None,  # noqa: ARG001
) -> dict[str, float]:
    """Diagnostics for `autosim.simulations.KolmogorovFlow`."""
    data = payload["data"]
    if not isinstance(data, torch.Tensor) or data.ndim != 5:
        raise ValueError("Expected payload['data'] as [b,t,n,n,4] torch.Tensor")
    if simulator is None:
        raise ValueError(
            "kolmogorov_flow_diagnostics requires simulator=KolmogorovFlow"
        )
    ...
    idx_psi = _channel_index_by_name(simulator, "streamfunction")
    if idx_w is None:
        raise ValueError("Missing vorticity channel")

    w = data[..., idx_w]
    out: dict[str, float] = {
        "omega_abs_max": float(w.abs().max().item()),
        "omega_l2": float(torch.linalg.vector_norm(w.float()).item()),
    }
...
def kolmogorov_flow_residual(
    payload: dict[str, Any],
    *,
    simulator: Any | None = None,
    simulator_kwargs: dict[str, Any] | None = None,  # noqa: ARG001
) -> dict[str, float]:
    """Residual for Kolmogorov flow vorticity equation.

    PDE: ω_t + u·∇ω = ν∇²ω + f_ω(y) - α ω
    with f_ω(y) = -forcing * kf * cos(kf y).
    """
    data = payload["data"]
    if not isinstance(data, torch.Tensor) or data.ndim != 5:
        raise ValueError("Expected payload['data'] as [b,t,n,n,c] torch.Tensor")
    if data.shape[1] < 2:
        raise ValueError("Need at least 2 time steps to compute a time residual")
    if simulator is None:
        raise ValueError("kolmogorov_flow_residual requires simulator=KolmogorovFlow")
    ...
    dx = L / float(n)
    dt = float(getattr(simulator, "dt", 1.0))
    kf = int(getattr(simulator, "kf", 4))

    nu = _get_param(payload, simulator, "nu")
    forcing = _get_param(payload, simulator, "forcing")
    alpha = _get_param(payload, simulator, "alpha")
    ...
    # Forcing in vorticity depends on y only.
    y = torch.linspace(0.0, L, n + 1, device=data.device, dtype=data.dtype)[:-1]
    f_omega = (-forcing * float(kf) * torch.cos(float(kf) * y)).reshape(1, 1, 1, n)

    rhs = -u_t * dw_dx - v_t * dw_dy + nu * lap_w + f_omega - alpha * w_t
    r = dw_dt - rhs
